...
import { FlashcardPreferences } from "@/components/flashcard/FlashcardOptions";

export const useFlashcardsWithPreferences = (preferences: FlashcardPreferences) => {
  ...
  const saveFlashcards = async (deckName: string) => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: `"${deckName}" saved with ${flashcards.length} cards!`,
      });
    } catch (error) {
